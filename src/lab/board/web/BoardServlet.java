...
public BoardServlet()
{
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		String url = "";//get방식으로 parameter를 보낼때! 
		
		if("write".contentEquals(action))
		{
		...
		}else if("list".equals(action))
		{
			...
			int bbsCount = dao.selectTotalArticleCount();//게시글 전체 개수
			int totalPage = 0;
			if(bbsCount > 0)
			{
		...
		else if("view".contentEquals(action))
		{
			...
			{
				board.setContent(board.getContent().replaceAll("/n", "<br>"));
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
	{
		...
		if("write_do".contentEquals(action))
		{
		...
		else if("update_do".contentEquals(action))
		{
			String password = request.getParameter("password");
			String bbsnoStr = request.getParameter("bbsno");
		...
		{
			request.setAttribute("message", "잘못된 명령: doPoast-" + action);
			url = url + "/error/error.jsp";
		}
		RequestDispatcher disp = request.getRequestDispatcher(url);
		disp.forward(request,response);
